...
void setup() {
  ...
  if (GPIOLEDDIST > 0)
    pinMode(GPIOLEDDIST, OUTPUT);
  if (GPIOLEDTEMP > 0)
    pinMode(GPIOLEDTEMP, OUTPUT);
...
void updateWheelPos(void) {
  ...
  if (wheelPosCode >= 7)
    wheelPosCode = DEVICENAMECODE;  // set from configuration
...
int getVbat(void) {
  ...
  const int vref = 1100;
  const float voltageCorrectionFactor = 0.950;

  return (averageInputValue / 4096.0) * 2.0 * 3.3 * vref * voltageCorrectionFactor;
